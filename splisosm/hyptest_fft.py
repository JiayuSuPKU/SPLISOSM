...
def _sv_worker_fft(
    ...
    y_cube = y_cube - y_cube.mean(axis=(0, 1), keepdims=True)
...
class SplisosmFFT:
    ...
    def extract_feature_summary(
        ...
        if level == "gene" and self._gene_summary is not None:
            return self._gene_summary
        if level == "isoform" and self._isoform_summary is not None:
        ...
        if (
            is_sparse
            and not sp.isspmatrix_csc(iso_counts)
            and not sp.isspmatrix_csr(iso_counts)
        ):
        ...
        if level == "gene":
            rows = []
            for gene_name, iso_names in zip(self.gene_names, self._gene_iso_names):
                iso_idx = var_names_index.get_indexer(iso_names)
                _counts = iso_counts[:, iso_idx]

                if is_sparse:
                    _sum_per_iso = np.asarray(_counts.sum(0)).ravel()
                    _row_sums = np.asarray(_counts.sum(1)).ravel()
                else:
                    _counts_arr = np.asarray(_counts)
                    _sum_per_iso = _counts_arr.sum(0)
                    _row_sums = _counts_arr.sum(1)

                _total = float(_row_sums.sum())

                if _total == 0.0:
                    _ratios_avg = np.zeros(len(iso_names), dtype=float)
                else:
                    _ratios_avg = _sum_per_iso.astype(float) / _total
                ...
                rows.append(
                    {
                        ...
                        "pct_bin_on": float((_row_sums > 0).mean()),
                        "count_avg": float(_row_sums.mean()),
                        "count_std": float(_row_sums.std()),
                    }
                )

            self._gene_summary = pd.DataFrame(rows).set_index("gene")
            return self._gene_summary
        ...
        for iso_names in self._gene_iso_names:
            iso_idx = var_names_index.get_indexer(iso_names)
            _counts_gene = iso_counts[:, iso_idx]

            if is_sparse:
                _row_sums_gene = np.asarray(_counts_gene.sum(1)).ravel()
                _counts_dense = _counts_gene.toarray()
            else:
                _counts_dense = np.asarray(_counts_gene)
                _row_sums_gene = _counts_dense.sum(1)

            _total_gene = float(_row_sums_gene.sum())
            valid_mask = _row_sums_gene > 0

            for j, iso_name in enumerate(iso_names):
                _iso_counts = _counts_dense[:, j].astype(float)

                pct_bin_on = float((_iso_counts > 0).mean())
                count_total = float(_iso_counts.sum())
                count_avg = float(_iso_counts.mean())
                count_std = float(_iso_counts.std())

                if valid_mask.any() and _total_gene > 0:
                    _iso_valid = _iso_counts[valid_mask]
                    _gene_valid = _row_sums_gene[valid_mask].astype(float)
                    _ratios = _iso_valid / _gene_valid
                    ratio_total = float(_iso_counts.sum() / _total_gene)
                    ratio_avg = float(_ratios.mean())
                    ratio_std = float(_ratios.std())
                else:
                    ratio_total = 0.0
                    ratio_avg = 0.0
                    ratio_std = 0.0

                all_iso_names.append(iso_name)
                iso_rows.append(
                    {
                        "pct_bin_on": pct_bin_on,
        ...
        stats_df = pd.DataFrame(iso_rows, index=all_iso_names)
